...
function EnqueueStrategy({ value, onValueChange, className }: EnqueueStrategyProps) {
  return (
    <div
      data-slot="enqueue-strategy"
      className={cn(
        'mt-4 flex items-center justify-between border-t border-slate-200 pt-3 dark:border-slate-800',
        className,
      )}
    >
      <div>
        <label htmlFor="same-domain" className="block text-xs font-medium">
          stay on this domain
        </label>
